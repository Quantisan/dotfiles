...
**Load-bearing filter.** Not every assumption matters equally. For each extracted bet, apply the CIA's Key Assumptions Check: "If the assumption proves to be wrong, would it significantly alter the analytic line? How?" (CIA, 2009, p. 9). Retain only assumptions whose failure would require rethinking the design, not merely adjusting it.

The output is a list of the spec's bets — the assumptions it depends on but does not examine. This is not summarization; it is excavation.
...
- Klein, G. (2007). "Performing a Project Premortem." *Harvard Business Review*, 85(9), 18–19. — Pre-mortem technique: assume the project has already failed, then explain why. Shifts reasoning from confirmatory to explanatory by treating failure as certain rather than hypothetical.
- Mitchell, D. J., Russo, J. E., & Pennington, N. (1989). "Back to the Future: Temporal Perspective in the Explanation of Events." *Journal of Behavioral Decision Making*, 2(1), 25–38. — Prospective hindsight (certain outcomes) produced significantly more reasons than uncertain framing (4.60 vs. 3.28 per scenario, F(1,99) = 19.92, p < .001); the mechanism is outcome certainty shifting reasoning strategy, not temporal imagination per se.
- Newell, G. E., et al. (2010). "Taming the Warrant in Toulmin's Model of Argument." *English Journal*, 99(6), 41–46. — Warrants "usually remain implicit in an argument; they are the unspoken assumptions that bind together claims and data" and "no amount of close reading reveals them; instead, they must be brought to the surface through logical inference" (p. 41).
- Toulmin, S. E. (1958). *The Uses of Argument.* Cambridge University Press. — Defines the warrant as the implicit general principle bridging data to conclusion: "general, hypothetical statements, which can act as bridges, and authorise the sort of step to which our argument commits us" (p. 98); "data are appealed to explicitly, warrants implicitly" (p. 100).
