...
- Each finding must quote the specific spec text it challenges before stating the finding. No quote, no finding. This is a structural constraint, not a style preference — it mechanically prevents ungrounded findings and forces each claim to a verifiable grain size (evidence-anchored review; Anthropic, 2025; Hong et al., 2026; Bai et al., 2024).
- Before reviewing, search for established frameworks from your field that apply to this spec's domain. Search for each independently. Each framework you apply must cite a specific source (author, year, title). No citation, no framework. This is a structural constraint parallel to the quote requirement for findings — it mechanically prevents reviewers from invoking impressive-sounding methods they cannot substantiate (perspective-driven search decomposition; Shao et al., 2024). Do not accept the first plausible search result; evaluate whether each retrieved source is relevant to the problem and whether your use of it is supported by what the source actually says. Proceed only when you have a citable, verified method to apply (evidence sufficiency gating; Asai et al., 2023).
