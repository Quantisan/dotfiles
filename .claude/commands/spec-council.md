...
Read the HTML template from `~/.claude/commands/templates/spec-council-review.html`. Use it as the structure for the output — populate the bracketed placeholders with actual review content. No CSS frameworks, no JavaScript, no external assets.

Write the rendered HTML to the output path computed at the start of this run. Overwrite if the file already exists.
