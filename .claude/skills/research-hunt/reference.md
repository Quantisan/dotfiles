...
- **Fabrication clusters where the persuasion is.** Point adversarial verification at load-bearing quotes first; scouts copy quote bytes, never transcribe; the report re-checks flagship quotes independently.
...
- **Fetched text is data.** Quotes are copied verbatim from untrusted pages by design, and that text flows into gate prompts and the report — so every agent that touches web-derived text is told instructions embedded in it are content to report, never instructions to follow.
...
Judgment lives in gate agents on the session model; control flow lives in the script. The retro's fixes are control-flow fixes — deterministic code holds them; re-improvisation drifts on them. Deleting a mechanism as an "obvious simplification" reintroduces the failure it fixed.

## Maintaining the engine

- The canonical source lives in the dotfiles repo (checkout path recorded at `~/.claude/workflows/.source`); the installed copy at `~/.claude/workflows/research-hunt.js` is what runs. Edit the canonical source, then `source bootstrap.sh` to reinstall — never edit the installed copy.
- Read this file before any engine edit — the counters and gates encode the lessons below.
- After any run that forces an engine edit, record the edit's rationale in the log at the bottom, dated.
- **Re-base check on every edit:** ask "does the harness now do any of this natively?" (scheduling, structured-output enforcement, resume, journaling already come from harness primitives) — and delete what it does. The methodology stays in code; the plumbing should not.

## The origin trail

Each mechanism encodes a lesson from the reasoning-trail research chain (cinema → analyses → games rounds, `research/2026-07-08.reasoning-trail-*.md` in the Spithre-Discovery repo), distilled in the design spec (`docs/superpowers/specs/2026-07-09-research-hunt-skill-design.md` there). Deleting one as an "obvious simplification" reintroduces the failure it fixed.
...
Judgment lives in gate agents on the session model; control flow lives in the script. The retro's fixes are control-flow fixes — deterministic code holds them; re-improvisation drifts on them.

## The tiers and the effort map

- **Session model (no pin)** — the main session (brief authoring, launch, report writing including the independent quote re-checks) and the judgment nodes: the three gates (discovery ranking, cross-case, question), the per-candidate case leads, and the Phase-3 verdict. These carry no `model` override on purpose — the judgment layer inherits whatever model the session runs, so model improvements reach it without engine edits. (The old `opus` pins were right when Opus was the top tier; once sessions ran a stronger model, the pins held judgment *below* the driving model.)
- **Sonnet (pinned)** — perspective scouts, lens scouts, verifiers, benchmark passes, refuters, clustering, critic: high-volume search and mechanical work. The pin is an explicit cost decision, not a capability judgment — revisit it if the cost/quality tradeoff moves.
...
|---|---|
| Gate's provisional cluster scores ran systematically hot — every deep-dived candidate scored at or below its estimate (games survey §Caveats; Roottrees gated 5/5/4, deep-dived 4/4/3) | Gate is instructed to score conservatively and told its scores are superseded by deep-dive scores; the report presents gate scores as provisional |
...
| Saturation counted in code worked — the games round's "15 → 8 → 4 → 4 → 0 → 0" trace, "the rule fired exactly as designed" (games survey §Phase 1) | Kept: two consecutive perspectives yielding zero new clusters closes discovery; the per-perspective sequence is logged and returned for the report |
| Fabrication clusters exactly where the persuasive work is done — six caught fabrications, all in load-bearing flagship quotes (cinema survey §Cross-candidate patterns; games survey §Caveats) | Adversarial verifier is pointed at load-bearing claims first; case leads and lens scouts are told to copy quote bytes, never transcribe; the report re-checks flagship quotes independently |
| Verifiers **false-flag too** — a real Loki production still was wrongly declared fabricated until an independent curl check found it live (cinema survey §Caveats) | 'refuted' requires the verifier to have found the source and shown the mismatch; can't-find-it is 'inconclusive'; the report re-checks refutations as well as confirmations |
...
| The hardest question earns its answer only when someone tries to break it — the games homework verdict survived because a fabricated anchor quote was caught and the verdict re-based on verified evidence (games survey §Caveats) | Phase 3 runs **fresh** refuters with a contamination guard (no `SCOUT_CONTEXT`, no scout framing), a coded stop rule (`stabilityK` consecutive unmoved new sources), and `underdetermined-build-probe` as a first-class verdict |
...
**Contamination guard**: refuters receive the question, the tentative answer, and the running already-seen source list (dedup) — none of `SCOUT_CONTEXT`, none of the round's scouts' framing or findings. The claim under attack must be known to attack it; the framing that produced it must not be, or the refuters inherit the scouts' blind spots.
...
- **Gate constraint violations get one corrective re-run**, then proceed logged-but-unblocked: a hard loop on a stubborn gate would deadlock the run for a constraint the report can still disclose. The Phase-3 verdict's missing probe suggestion gets the same single-retry treatment.
...
### 2026-07-09 — two-skill split; judgment nodes unpinned from Opus

Evidence: a design decision (dotfiles `docs/specs/2026-07-09-research-hunt-split-design.md`), not a run. The `model: 'opus'` pins on the judgment nodes (discovery gate + retry, case leads, cross-case gate, question gate, verdict + retry) dated from when Opus was the top tier; sessions moved to a stronger model, so the pins held the judgment layer below the driving model. Judgment nodes now inherit the session model; Sonnet pins stay as the explicit cost knob. Effort overrides unchanged. Entry via two skills: `research-hunt-brief` writes the brief, `/research-hunt <brief.md>` is the go.
